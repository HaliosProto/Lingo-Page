...
Milestone 2 adds 68 named deterministic cases for recovery decisions, expiry/pruning, navigation identity, mutation backpressure, request idempotency, persisted retry deadlines, network categories, and completed-ID filtering. Managed Chromium additionally reloads a translated page, proves zero-call reconstruction, replaces the root with compatible hydration, applies a 600-node mutation storm, and verifies an SPA content-identity change remains original. Physical sleep/wake, branded Chrome, and forced-GC evidence remain separate manual gates.
