...
| Extension access | MV3 required permissions are `activeTab`, `contextMenus`, `scripting`, and `storage`; the local API is the only required production host. Optional HTTP/HTTPS patterns are granted only per current origin for translated copies. |
...
- Generated production manifest and bundled JavaScript are included in the permission/secret audit. Optional HTTP/HTTPS declarations grant nothing automatically; the required fixture host appears only in `.output/chrome-mv3-e2e`.
