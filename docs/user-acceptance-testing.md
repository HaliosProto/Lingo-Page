...
- Choose **Check for new or changed content** without changing the page. Expected: a visible “No page changes found” result and no translation request. Repeat in both Original and Translated views. Add, modify, remove, and reorder visible text on a safe synthetic page, then scan again. Expected: new, modified, removed, reordered, and uncertain counts are clear; uncertain text stays original. **Update changed sections** sends only confident new or modified text. **Refresh translation** is a separate confirmed quota-using action; **End translation session** restores and discards only the active page session.
- Choose **Open translated copy** on both an HTTP and an HTTPS test page. Expected: the popup first says “Lingo Page needs access to this site to open an automatically translated copy.” Chrome requests access only to that page's origin, not every site. Grant access. Exactly one visible destination tab opens and remains open; after any bounded page hydration, confidently matched translations are visible before readiness is reported, unmatched text remains original, the reuse summary is visible, and no provider request occurs. A same-origin redirect may reuse the handoff; a cross-origin redirect or revoked permission fails closed before injection and leaves the destination open with a safe status. Ending or closing the source does not change the copy, and closing the copy does not change the source.
- On a second safe origin, choose **Open translated copy** and deny access. Expected: no destination is created, the source session and translated DOM remain unchanged, a second click in the same popup does not prompt again, and the popup explains that the user can duplicate the tab and invoke Lingo Page manually there. Reopen the already-granted origin and verify it opens without a new permission prompt. In `chrome://extensions`, revoke that site's access and verify the next copy attempt cannot inject until the user explicitly grants that origin again.
- Choose **Open comparison view**. Expected: a compact toolbar and two recognizable full-height page representations appear, Original left and Translation right at approximately 50/50. Verify linked scrolling in both directions, unlink/relink, pointer and keyboard divider adjustment, Reset, Swap sides, light/dark/reduced motion, 200% zoom, and the narrow stacked fallback. Source scripts, handlers, forms, fields, frames, and embeds must not appear or execute; opening and operating comparison must not create a translation request.
