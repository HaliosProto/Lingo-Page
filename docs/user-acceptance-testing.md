...
- Choose **Open translated copy**. Expected: exactly one visible destination tab opens and remains open, confidently matched translations appear without provider requests, unmatched text remains original, and a reuse summary is visible. A redirected or mismatched handoff leaves the destination tab open with a safe failure status. Ending or closing the source does not change the copy, and closing the copy does not change the source.
