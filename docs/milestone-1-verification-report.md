...
- Beads parent: `translation-1mp.2.11`; bugs `.11.1`, `.11.2`, `.11.3`, and translated-copy permission/hydration follow-up `translation-1mp.2.12`
...
The optional-permission/hydration follow-up performance run completed on 2026-07-18. Its 2,206-segment fixture reached first visible in 111 ms and completion in 480 ms; Original and Translated switches took 31 ms and 35 ms, ten cycles took 288 ms, and no switch provider call or long task occurred. Copy matching/application completed in 2,058 ms because the acceptance path now includes the bounded hydration window; comparison loaded in 474 ms, and copy plus comparison made zero provider calls. Dynamic insertion completed in 823 ms.

## Automated and runtime evidence

- Static/application gate: focused changed-file formatting, ESLint, strict type checks, 105 unit/regression tests, 21 explicit API integration tests, production extension build, and Worker dry-run build passed. Repository-wide formatting still reports the pre-existing baseline listed below.
...
- In the optional-permission follow-up session, direct repository formatting reported nine unchanged baseline files. The package-manager wrapper and release-candidate script then stalled before producing output and were terminated; their underlying lint, type, test, build, browser, manifest, and security gates were run directly and passed. The focused changed-file formatting check passed. No new release-candidate claim is made.
