...
- Page-owned sessions support repeated views, explicit change scans, changed-only updates, acknowledged copy isolation, and sanitized full-page comparison while the content script remains alive. Full service-worker/browser restart, sleep/wake, reload reconstruction, and complex SPA navigation persistence are intentionally deferred.
...
- The 2026-07-18 production dependency audit reports one high advisory: `adm-zip@0.5.18` through WXT → `web-ext-run` → `firefox-profile`. This path is build/development tooling and is not included in the produced Chrome extension, but the lockfile remains affected until the WXT chain adopts `adm-zip >=0.6.0` or a separately approved compatible override is verified.
- `pnpm verify` and release-candidate packaging currently stop at a pre-existing merged-main Prettier baseline affecting 14 unchanged files. Every file changed by the acceptance fix passes focused formatting, and lint/types/tests/builds pass. Resolving the unrelated formatting baseline needs a separately approved mechanical cleanup.
