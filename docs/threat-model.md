...
| Overbroad or stale copy permission   | Extension can inject outside the site the user selected        | Optional HTTP(S) declaration only; direct-gesture exact-origin request; checks before tab creation and after final navigation; no all-site request          | Granted, denied, already-granted, HTTP/HTTPS, wrong-origin, redirect, and revocation tests. |
