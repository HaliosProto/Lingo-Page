...
A translated-copy action creates a bounded temporary tab-bound `chrome.storage.session` handoff. The destination page cannot read that isolated storage, the URL carries no bundle or token, and central page-text data is removed after destination acknowledgment or safe failure cleanup. A comparison action creates a separate bounded owning-tab bundle; its URL carries only a random single-use token. Its structural snapshot excludes forms, editable content, hidden content, input values, source scripts, event handlers, and arbitrary styles. Safe image loads and links may contact their resolved HTTP(S) origins with referrer suppression only when represented in the captured reading structure. No history, analytics, account record, cloud session store, or broad permission was added.
