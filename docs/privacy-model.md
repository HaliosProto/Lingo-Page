...
Milestone 2 active-session recovery is local-only, off in privacy mode, and limited to 30 minutes. The record contains navigation/session identities, fingerprints, status, and completed translated values but excludes raw source/original text, HTML, titles, full URLs, inputs, hidden content, cookies, and provider payloads. It is exact-tab bound and automatically removed on terminal or incompatible lifecycle events. Recovery performs no provider call and therefore introduces no new recipient.
