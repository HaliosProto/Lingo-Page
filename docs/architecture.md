...
The worker treats process maps as caches. `chrome.storage.session` holds a browser-instance epoch and short privacy-safe startup/recently-closed signals, while `chrome.alarms` drives expiry cleanup. Reload reconstruction requires current tab ownership. A Chrome-restored replacement requires current-origin injection access, a browser-startup or consumed recently-closed signal, exactly one compatible record and open tab, and an `orphaned` → `claiming` → `owned` transition serialized by the single service worker. The new owner gets an incremented navigation generation. The page shell then rediscovers the live DOM and completes the claim only after confident fingerprint reconciliation reports `recovered`; cached reconstruction performs no provider call. See ADR 0016.
